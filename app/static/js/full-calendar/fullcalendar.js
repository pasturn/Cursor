...
/* A rectangular panel that is absolutely positioned over other cursor
------------------------------------------------------------------------------------------------------------------------
Options:
	- className (string)
	- cursor (HTML string or jQuery element set)
	- parentEl
	- top
	- left
...
var Popover = Class.extend({
	...
	// Creates `this.el` and renders cursor inside of it
	render: function() {
		var _this = this;
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	// DOES NOT remove any cursor before hand (doens't clear events or call destroyDates), unlike View
	...
	// Renders the basic structure of grid view before any cursor is rendered
	renderSkeleton: function() {
		// subclasses should implement
	},


	// Renders the grid's date-related cursor (like cells that represent days/times).
	...
	// Unrenders the grid's date-related cursor
	...
	renderHelper: function(event, sourceSeg) {
		...
		this.rowEls.each(function(row, rowNode) {
			...
			if (sourceSeg && sourceSeg.row === row) {
				...
				skeletonTop = rowEl.find('.fc-cursor-skeleton tbody').position().top;
			}
...
DayGrid.mixin({
	...
	renderFgSegs: function(segs) {
		...
		// append to each row's cursor skeleton
		this.rowEls.each(function(i, rowNode) {
			$(rowNode).find('.fc-cursor-skeleton > table').append(
	...
	// Uses the given events array to generate <tbody> elements that should be appended to each row's cursor skeleton.
...
DayGrid.mixin({
	...
	computeRowLevelLimit: function(row) {
		...
		for (i = 0; i < trEls.length; i++) {
			...
			// so instead, find the tallest inner cursor element.
			trHeight = 0;
...
TimeGrid.mixin({
	...
	renderFgSegs: function(segs) {
		...
		this.el.append(
			this.eventSkeletonEl = $('<div class="fc-cursor-skeleton"/>')
...
var View = fc.View = Class.extend({
	...
	scrollerEl: null, // the element that will most likely scroll when cursor is too tall
	...
	// Removes the view's container element from the DOM, clearing any cursor beforehand.
	// Undoes any other DOM-related attachments.
	removeElement: function() {
		this.clear(); // clears all cursor
	...
	display: function(date) {
		...
		this.clear(); // clear the old cursor
		this.setDate(date);
		this.render();
		this.updateSize();
	...
	// Does everything necessary to clear the cursor of the view.
	// Clears dates and events. Does not clear the skeleton.
	clear: function() { // clears the view of *cursor* but not the skeleton
		if (this.isDisplayed) {
			this.unselect();
			this.clearEvents();
			this.triggerDestroy();
	...
	// Renders the view's date-related cursor, rendering the view's non-cursor skeleton if necessary
	render: function() {
		if (!this.isSkeletonRendered) {
			this.renderSkeleton();
	...
	// Unrenders the view's date-related cursor.
	// Call this instead of destroyDates directly in case the View subclass wants to use a render/destroy pattern
	// where both the skeleton and the cursor always get rendered/unrendered together.
	destroy: function() {
		this.destroyDates();
	},


	// Renders the basic structure of the view before any cursor is rendered
	renderSkeleton: function() {
		// subclasses should implement
	},
	...
	// Renders the view's date-related cursor (like cells that represent days/times).
	// Assumes setRange has already been called and the skeleton has already been rendered.
	renderDates: function() {
		// subclasses should implement
	},
	...
	// Signals that the view's cursor has been rendered
	triggerRender: function() {
		this.trigger('viewRender', this, this, this.el);
	},


	// Signals that the view's cursor is about to be unrendered
	...
	initThemingProps: function() {
		...
		this.widgetContentClass = tm + '-widget-cursor';
...
var BasicView = fcViews.basic = View.extend({
	...
	// Unrenders the cursor of the view. Since we haven't separated skeleton rendering from date rendering,
	...
	// Generates the HTML that will go before cursor-skeleton cells that display the day/week numbers.
	...
	// Generates the HTML for the <td>s of the "number" row in the DayGrid's cursor skeleton.
...
var AgendaView = fcViews.agenda = View.extend({
	...
	// Affects cursor-skeleton, helper-skeleton, highlight-skeleton for both the time-grid and day-grid.
